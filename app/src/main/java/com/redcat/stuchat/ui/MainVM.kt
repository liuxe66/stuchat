...
import com.redcat.stuchat.ext.differDay
...
class MainVM : BaseViewModel() {
    ...
    private var luxunList: List<String> by PrefUtils(PrefUtils.prefluxunList, ArrayList())
    private var luxunIndex by PrefUtils(PrefUtils.prefluxunIndex, 0)

    private var prefLastTime by PrefUtils(PrefUtils.prefLastTime, 0L)
    ...
    fun initUser() = liveData<UserBean> {
        ...
        var listdata = ArrayList<UserBean>()
        ...
        LogUtils.e("userListData:" + userListData.size)
        emit(user)
    }

    fun updateUserList() = liveData {
        var userList = userListData
        if (prefLastTime == 0L) {
            emit("ok")
            userList.forEach { userBean ->
                userBean.frame = creatFrame()
                userBean.veh = creatVeh()
                userBean.wordNum = userBean.wordNum + (1 until 5).random()
            }
        } else {
            var days = System.currentTimeMillis().differDay(prefLastTime)
            if (days > 0) {
                userList.forEach { userBean ->
                    userBean.frame = creatFrame()
                    userBean.veh = creatVeh()
                    userBean.wordNum = userBean.wordNum + days * (8 until 20).random()
                }
            } else {
                userList.forEach { userBean ->
                    userBean.frame = creatFrame()
        ...
        prefLastTime = System.currentTimeMillis()
    ...
    fun loadWord(fileName: String = "word.json") = liveData<String> {
        withContext(Dispatchers.IO) {
            ...
            var tempWordsList: MutableList<WordBean> = gson.fromJson(str, wordType)
            val iterator: MutableIterator<WordBean> = tempWordsList.iterator()
            while (iterator.hasNext()) {
                val word = iterator.next()
                if (word.name!!.length < 3) iterator.remove()
            }
            tempWordsList.shuffle()
            wordsList = tempWordsList
        }

        // 解析Json数据 鲁迅
        val newstringBuilder1 = StringBuilder()
        var inputStream1: InputStream? = null
        try {
            inputStream1 = RedCatApp.context.assets.open("luxun.json")
            val isr = InputStreamReader(inputStream1)
            val reader = BufferedReader(isr)
            var jsonLine: String?
            while (reader.readLine().also { jsonLine = it } != null) {
                newstringBuilder1.append(jsonLine)
            }
            reader.close()
            isr.close()
            inputStream1.close()
        ...
        val str1 = newstringBuilder1.toString()
        val gson1 = Gson()
        val wordType1: Type = object : TypeToken<List<String>>() {}.type
        luxunList = gson1.fromJson(str1, wordType1)
    ...
    fun addTimestamp(record: Record) {
        if (record.timestamp - lastShowTimeStamp > 300000) {
            ...
            recordList.add(
                RecordBean(
                    type = AppConfig.type_timestamp, showTimestamp = lastShowTimeStamp
                )
            )
        }
        recordList.add(
    ...
    fun sayHello() = viewModelScope.launch {
        if (isFirst) {
            ...
            var hello1 = "我们的自习室，主要目标是学习英语单词，每天任务是学习10个单词。"
     ...
     * 随机创建用户
     */
    fun createUser(): UserBean {
        return userListData[(0 until 9).random()]
    }
     ...
     */
    fun creatAvatar(): Int {
        return return (1 until 8).random()
    }
    ...
    fun creatWordNum(): Int {
        var num = (1 until 10).random() * (10 until 20).random()
        return num
    }
    ...
    fun creatAQ() {
        insertRecord(
            AppConfig.type_sys_text, text = aqList[aqIndex], avatar = 100, nickName = "鲁迅"
        )
        aqIndex++
    }
