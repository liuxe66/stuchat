...
import com.redcat.stuchat.data.bean.WordLevelQuestionFillBean
import com.redcat.stuchat.data.bean.WordLevelQuestionSelectBean
...
class WordVM : BaseViewModel() {
    ...
    private var wordsList: MutableList<WordBean> by PrefUtils(PrefUtils.prefWordList, ArrayList())
    private var wordsLevel by PrefUtils(PrefUtils.prefWordLevel, 1)

    var curQuestionLiveData = MutableLiveData<WordLevelQuestionSelectBean>()
    ...
    fun creatSelectQuestion(index: Int) {
        var index1 = (100 until 3000).random()
        var index2 = (100 until 3000).random()
        var index3 = (100 until 3000).random()

        while (index1 == index2) {
            index2 = (100 until 3000).random()
        }

        while (index1 == index3 || index2 == index3) {
            index3 = (100 until 3000).random()
        }

        var selectList = ArrayList<String>()
        selectList.add(wordsList[index1].trans?.get(0)!!)
        selectList.add(wordsList[index2].trans?.get(0)!!)
        selectList.add(wordsList[index3].trans?.get(0)!!)
        selectList.add(getLevel()[index].trans?.get(0)!!)
        selectList.shuffle()

        curQuestionLiveData.value = WordLevelQuestionSelectBean(
            question = getLevel()[index].name!!,
            selectList = selectList,
            answer = getLevel()[index].trans?.get(0)!!
        )
    }
